...
function* watchUpdateUserProgress() {
  yield takeLatest(updateUserProgressAction.type, function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
    const { data, onError, onSuccess } = payload
    try {
      const res = yield call(updateUserProgressApi, { setId: data.setId, cardId: data.cardId });
...
function* watchGetUserLearningSetProgress() {
  yield takeLatest(getUserLearningSetProgressAction.type, function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
    const { data, onError, onSuccess } = payload
    try {
...
function* watchGetUserLearningProgress() {
  yield takeLatest(getUserProgressAction.type, function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
