...
function* watchGetQuestionsListBySetId() {
  yield takeLatest(
    getQuestionsListBySetIdAction.type,
...
function* watchEditQuestion() {
  yield takeLatest(
    editQuestionAction.type,
    function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
      const { data, questionId, testKitId, onError, onSuccess } = payload;
      try {
        const res = yield call(editQuestionApi, {
...
function* watchCreateQuestion() {
  yield takeLatest(
    createQuestionAction.type,
    function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
      const { data, onError, onSuccess, testId } = payload;
      try {
        const res = yield call(createQuestionApi, {
          testId: testId,
          data: data,
        });
        if (res.status === ErrorCode.OK) {
...
function* watchCreateTestKit() {
  yield takeLatest(
    createTestKitAction.type,
    function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
      const { level, onError, onSuccess, setId } = payload;
      try {
        const res = yield call(createTestKitApi, {
          setId: setId,
          level: level,
        });
        if (res.status === ErrorCode.OK) {
