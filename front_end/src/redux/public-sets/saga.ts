...
import { GetSetsApi } from '@/api/SetsApi';
    ...
    function* ({ payload }: PayloadAction<any>): Generator<any, void, any> {
      ...
      try {
        ...
        isFunction(payload.onError) &&
          payload.onError(error?.response?.data?.message || 'Error');
        yield put({
...
export default function* publicSetsSaga() {
  yield all([fork(watchGetPublicSets)]);
}
