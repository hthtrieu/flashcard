...
export const approveSetApi = async ({ setId, level }: { setId: string, level?: number }) => {
    const response = await AxiosConfig.post(`${BASE_URL}/approve`, { setId, level });
    return response;
}
...
export const getSetByAdminApi = async (setId: string) => {
    const response = await AxiosConfig.post(`${BASE_URL}/get-set`, { setId: setId });
    return response;
}
